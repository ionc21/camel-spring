...
public class NewWebsiteOrderRouteTest {
	...
	public void setUp() throws Exception {
		...
		jdbcTemplateDerby.execute("insert into orders.catalogitem (id, itemnumber, itemname, itemtype) "
				+ "values (1, '078-1344200444', 'Build Your Own JavaScript Framework in Just 24 Hours', 'Book')");
		jdbcTemplateDerby.execute("insert into orders.customer (id, firstname, lastname, email) " + "values (1, 'Larry', 'Horse', 'larry@hello.com')");
	}
	...
	public void testNewWebsiteOrderRouteSuccess() throws Exception {
		jdbcTemplateDerby.execute("insert into orders.\"order\" (id, customer_id, orderNumber, timeorderplaced, lastupdate, status) "
				+ "values (1, 1, '1001', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'N')");
		jdbcTemplateDerby.execute("insert into orders.orderitem (id, order_id, catalogitem_id, status, price, quantity, lastupdate) "
				+ "values (1, 1, 1, 'N', 20.00, 1, CURRENT_TIMESTAMP)");
		Thread.sleep(5000);
		int total = jdbcTemplateDerby.queryForObject("select count(id) from orders.\"order\" where status = 'P'", Integer.class);
